    ...
    <table>
        ...
        <?php
            if($page>1){    //첫 페이지면 이전으로 못가게
                ?>
                <a href="<?=$_SERVER['PHP_SELF']?>?query=<?=$genre?>&page=<?=$page-1?>">이전</a>
            <?php
            }
            else{
                ?>
                <a href="<?=$_SERVER['PHP_SELF']?>?query=<?=$genre?>&page=<?=$page?>">이전</a>
            <?php
            }
        for($p=$s_page;$p<=$e_page;$p++){   //페이지 수 띄우기
            if($p==$page){
                ?>
                <a href="<?=$_SERVER['PHP_SELF']?>?query=<?=$genre?>&page=<?=$p?>"><b><?=$p?></b></a>
            <?php
            }
            else{
            ?>
                        <a href="<?=$_SERVER['PHP_SELF']?>?query=<?=$genre?>&page=<?=$p?>"><?=$p?></a>
                        <?php
            ...
            if($page<$pageNum){  //마지막 페이지면 다음으로 못가게
                ?>
                <a href="<?=$_SERVER['PHP_SELF']?>?query=<?=$genre?>&page=<?=$page+1?>">다음</a>
            <?php
            }
            else{
                ?>
                <a href="<?=$_SERVER['PHP_SELF']?>?query=<?=$genre?>&page=<?=$page?>">다음</a>
            <?php } ?>
    </p>            
            <?php
            $s_point = ($page-1) * $list;
